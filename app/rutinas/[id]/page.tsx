...
import { routineLevelLabels, routineObjectiveLabels } from '@/lib/routine-options'
...
export default async function RoutineDetailPage({ params }: RoutineDetailPageProps) {
  const [routine, profile] = await Promise.all([getPublishedRoutine(params.id), getProfile()])
  ...
  const nextStep = profile
    ? profile.role === 'OWNER'
      ? {
          title: 'Administra tus rutinas.',
          description: 'Edita la rutina, sus ejercicios o su estado desde el panel de administración.',
          href: '/dashboard/rutinas',
          label: 'Gestionar rutinas'
        }
      : {
          title: 'Registra tu progreso.',
          description: 'Guarda las series, repeticiones y el peso utilizado durante tu entrenamiento.',
          href: '/mi-rutina',
          label: 'Ir a mi rutina'
        }
    : {
        title: 'Guarda tu progreso.',
        description: 'Crea una cuenta para preparar tu espacio de entrenamiento en GymControl.',
        href: '/register',
        label: 'Registrarme'
      }
    ...
    <main className='bg-zinc-100'>
      <section className='bg-zinc-950 text-white'>
        <div className='mx-auto max-w-7xl px-5 py-14 sm:px-8 sm:py-20 lg:px-10'>
          <Link href='/rutinas' className='text-sm font-semibold text-lime-400 hover:text-lime-300'>
            Volver a rutinas
          </Link>
          <div className='mt-10 max-w-4xl'>
            ...
            <h1 className='mt-6 text-4xl font-black leading-tight sm:text-6xl'>{routine.nombre}</h1>
            <p className='mt-6 max-w-3xl text-lg leading-8 text-zinc-300'>{routine.descripcion}</p>
          </div>
        </div>
      </section>
      ...
      <section className='mx-auto grid max-w-7xl gap-8 px-5 py-14 sm:px-8 sm:py-20 lg:grid-cols-[1.4fr_0.6fr] lg:px-10'>
        <div className='bg-white p-7 sm:p-10'>
          ...
          <p className='mt-5 max-w-2xl leading-7 text-zinc-600'>
            Esta rutina combina un ritmo sostenible con una seleccion equilibrada de ejercicios para
            trabajar el objetivo de forma consistente durante toda la sesión.
          </p>
          <div className='mt-10 grid gap-px bg-zinc-200 sm:grid-cols-3'>
            ...
            <StatCard label='Objetivo' value={routineObjectiveLabels[routine.objetivo]} />
          </div>
                ...
                {routine.ejercicios.map((exercise) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      {exercise.grupoMuscular || exercise.equipamiento ? (
                        <p className='mt-1 text-xs text-zinc-500'>
                          {[exercise.grupoMuscular, exercise.equipamiento].filter(Boolean).join(' / ')}
                        </p>
                      ) : null}
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className='mt-1 font-semibold text-zinc-900'>{exercise.repeticiones}</p>
                    </div>
                  </article>
                ))}
              </div>
        ...
        <aside className='bg-lime-400 p-7 sm:p-8'>
          ...
          <h2 className='mt-4 text-2xl font-black text-zinc-950'>{nextStep.title}</h2>
          <p className='mt-3 text-sm leading-6 text-zinc-700'>{nextStep.description}</p>
          <Link
            href={nextStep.href}
