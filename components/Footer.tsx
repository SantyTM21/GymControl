    ...
    <footer className='border-t border-zinc-800 bg-zinc-950 text-zinc-400'>
      <div className='mx-auto grid max-w-7xl gap-10 px-5 py-12 sm:px-8 md:grid-cols-[1.4fr_1fr_1fr] lg:px-10'>
        ...
        <div>
          ...
          <div className='mt-4 flex flex-col gap-3 text-sm'>
            ...
            <Link href='/rutinas' className='hover:text-lime-400'>
              Rutinas
            </Link>
            <Link href='/ejercicios' className='hover:text-lime-400'>
              Ejercicios
            </Link>
          </div>
        </div>
        ...
        <div>
          ...
          <div className='mt-4 flex flex-col gap-3 text-sm'>
            {profile ? (
              <>
                <Link href='/perfil' className='hover:text-lime-400'>
                  Perfil
                </Link>
                <Link
                  href={profile.role === 'OWNER' ? '/dashboard' : '/mi-rutina'}
                  className='hover:text-lime-400'
                >
                  {profile.role === 'OWNER' ? 'Dashboard' : 'Mi rutina'}
                </Link>
              </>
            ) : (
              <>
      ...
      <div className='border-t border-zinc-800'>
        <div className='mx-auto flex max-w-7xl flex-col gap-2 px-5 py-5 text-xs sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-10'>
          ...
          <p>Hecho para avanzar, una repetición a la vez.</p>
        </div>
      </div>
    </footer>
  )
