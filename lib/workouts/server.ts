...
export async function listClientWorkoutRoutines() {
  ...
  const routines = await Promise.all(
    (data ?? []).map((routine) => getPublishedRoutine(routine.id)),
  );
...
export async function listClientWorkoutLogs() {
  ...
  const { data, error } = await supabase
    .from("workout_logs")
    .select(
      "id, client_id, routine_id, routine_exercise_id, performed_at, completed_sets, completed_reps, used_weight, created_at",
    )
